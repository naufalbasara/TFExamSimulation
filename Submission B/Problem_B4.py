...
def solution_B4():
    ...
    # YOUR CODE HERE
    # Using "shuffle=False"
    training_sentences, validation_sentences = train_test_split(bbc['text'], shuffle=False, train_size=training_portion)#YOUR CODE HERE
    training_labels, validation_labels = train_test_split(bbc['category'], shuffle=False, train_size=training_portion)#YOUR CODE HERE
    ...
    class CustomCallback(tf.keras.callbacks.Callback):
        def on_epoch_end(self, epoch, logs={}):
            if logs.get('accuracy')>0.91 and logs.get('val_accuracy')>=0.92:
                print("\nDesired accuracy reached, stop training")
                self.model.stop_training = True


    model.fit(
        padded_training, train_label_seq,
